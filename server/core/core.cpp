...
void Core::cancel_order(size_t user_id, size_t order_id) {
    if (order_book_.get_order_by_id(order_id).get_user_id() != user_id) {
        to_send_[user_id].push_back(Registry::get().error_response_);
        return;
    }
    order_book_.cancel_order(order_id);
    to_send_[user_id].push_back(order_book_.get_order_by_id(order_id).as_json());
}

void Core::view_book(size_t user_id) {
    nlohmann::json response;
    for (const auto &order: order_book_) {
